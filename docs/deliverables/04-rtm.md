...
| FR-NEWS-009 | §3.1.4 | **Implemented in W2-1** (`server.ts`): `stripNewsWorkflowFields` validation-layer strip of `externalSyncStatus`/`approvedBy`/`approvedAt`/`syncToExternal` on create/update; create always `'draft'` (no create/update sync log); submit-approval `'draft'`-only (else 400) with `news.submitted_by`/`submitted_at` persisted (schema.sql §3 + server DDL lockstep); approve/reject `'pending_approval'`-only (else 400) with submitter ≠ approver identity guard (else 403, admin included); **content change ⇒ draft** — editing any non-draft item (`'pending_approval'`/`'synced'`/`'rejected'`) resets it to `'draft'`, stamps cleared, dropped from the live sync set; guard rejections and forced transitions audited (AUD-P01/02/03). **W2-FIX-1 ext (commit `2c97cc3`, Doc 10 §9.3):** decisions on legacy no-submitter `pending_approval` rows denied 409 (fresh-cycle message) with `ACCESS_DENIED`/WARNING audit — dual control is unverifiable without `submittedBy`; all six news mutations serialize per item (`withNewsLock`); every workflow transition commits state+audit(+approve sync log) as one all-or-nothing unit (`commitNewsTransition`) — audit-write failure rolls back the transition (500), so no committed transition lacks its audit row | AS-BUILT (W2-1 + W2-FIX-1 ext) | TC-SEC-011; TC-NEWS-013/014 (legacy deny), TC-NEWS-017/018 (atomic rollback), TC-NEWS-019 (edit+approve race) — smoke §17, doc 12 v1.9.0 | — (TC-only) |
| FR-NEWS-010 | §3.1.4 | **Implemented in W2-FIX-1** (`server.ts:2006`): `POST /api/news/:id/withdraw` — maker/admin, **no checker** (ratified DCR-9: un-publish is the safe direction); body ignored (state-only — the server withdraws its *current* content, never a client snapshot); precondition `'synced'` else 409 with `currentState` (no audit row); transition `'synced'→'draft'` with `syncToExternal:false` and all four stamps cleared, **content preserved byte-for-byte**; `UPDATE`/SUCCESS audit with `prior_status='synced'` committed atomically (`commitNewsTransition`); no sync log. Client: `api.withdrawNews` (`src/api.ts`) behind the CMS "Withdraw from public" action (DCR-9 Option C) | AS-BUILT (W2-FIX-1, `2c97cc3`) | TC-NEWS-015 (409 precondition), TC-NEWS-016 (happy-path byte-fingerprint) — smoke §17, doc 12 v1.9.0 | — (TC-only) |
