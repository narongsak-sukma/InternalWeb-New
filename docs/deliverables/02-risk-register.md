...
**Version:** 1.3.0 · **Status:** Draft · **Date:** 2026-09-10 · **Author:** worker-1 → Lead review → CTO approval

> **Change log:** 1.3.0 — W2-5 code phase: RISK-010 Accepted (interim) → Mitigating — shared PostgreSQL-backed login-budget store shipped (`rate_limit_hits` schema in PG_DDL/`scripts/schema.sql` lockstep, fail-open WARNING with degradation counter, smoke §15 cross-process + fail-open proofs; README §8 and HANDOVER §10 rewritten as built); added terminal status value Mitigated (verified closure) per Lead-ratified W2-5 decision. 1.2.0 — added RISK-022 (DCR-7, dual-control state-machine bypass) and RISK-023 (DCR-8, audit-trail fabrication) per CTO gate REVISE Blocker 3; register now 23 risks. 1.1.0 — added RISK-021 (maker-checker bypass on `/api/news`, verified at `server.ts:1306`), lead-triaged as DCR-3, pending CTO ratification at the Wave-1 gate. 1.0.0 — initial register (20 risks).
...
- **Status values:** Open (no mitigation yet), Mitigating (control in force,
  ...
  scheduled in a wave), Mitigated (verified closure — the mitigation is
  merged, tested, and evidenced; the risk is retired unless re-identified).
...
| RISK-010 | Per-pod rate limiting: login throttling (5 attempts/min/IP) and failed-login tracking are in-memory per process; with replicas behind an ingress an attacker distributes guesses across pods, weakening brute-force protection. | security | M | M | 4 | W2-5 shipped: the failed-login budget is counted in the active repository — a PostgreSQL `rate_limit_hits` atomic upsert shared by every pod when `DATABASE_URL` is set (in-memory dev stays per-process); a transient store error fails open with a WARNING + per-process degradation counter (rationale in code + design note §8). Schema added in PG_DDL/`scripts/schema.sql` lockstep (idempotent, boot auto-migrate, W2-1 pattern); smoke §15 (opt-in `SMOKE_DATABASE_URL`) proves cross-process counting, out-of-process budget authority, and the fail-open path; README §8 + HANDOVER §10 rewritten as built. Flips to Mitigated (verified closure) once the W2-5 PR clears the merge gates and system-test evidence lands (design note `.omc/research/rate-limit-shared-store.md` §6). | Lead + IT admin | Mitigating |
