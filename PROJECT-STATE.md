...
| 2 | W2-6 sync webhook (FR-SYNC-004) design + k8s egress | worker-4 | design v0.3 DONE 2026-09-11 (.omc/research/w2-6-sync-webhook-design.md, sandbox-only): trigger map T1/T2/T4, correlationId/eventId traceability, HMAC+TLS+5-min-skew, retry×2 + FAILED ledger, honest-UI rules, k8s egress template. LEAD RULINGS on worker-4's flags: D5 AMENDMENT ACCEPTED (void dispatchWebhook() fire-and-forget with internal containment — amends ratified inline-await; approve latency must not couple to external health; rides to codex W2-GATE-2) · D6 correlationId ACCEPTED · DCR-10 RESOLVED (externalCategory excluded from payload → removal rides CONTENT_* migration, see §8). ADDENDUM DISPATCHED (v0.4): T5 withdrawal propagation per codex advisory — local draft reset must remove an already-delivered public copy (identity-minimal payload {id,title}, news.withdrawn). Code phase still external-blocked (codex PASS + W2-1 merge + public-web blockers 1-4) |
